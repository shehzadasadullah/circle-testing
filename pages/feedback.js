...
const Feedback = () => {
  ...
  const [creatorLoader, setCreatorLoader] = useState(true);
  const [ratingLoader, setRatingLoader] = useState(true);
  ...
  const handleFeedback = async () => {
    ...
    if (selectedEmoji === 0) {
      ...
      try {
        ...
        await fetch(
          "https://api.circle.ooo/api/circle/event/add-feedback",
          requestOptions
        )
          ...
          .then((result) => {
            ...
            if (result.result === true) {
              if (result.message === "event feedback added successfully") {
              ...
              if (
                result.message === `no event found with given id : ${eventid}`
              ) {
                toast("No Event Found!");
                setFeedbackLoader(false);
              } else if (
                result.message ===
                "current user is not the attendee of the event"
              ) {
                toast("You are not the attendee of this event!");
                setFeedbackLoader(false);
              } else if (
                result.message === "feedback already exsit for this user"
              ) {
                toast("You've already added review for this event!");
                setFeedbackLoader(false);
              } else {
                toast(result.message);
                setFeedbackLoader(false);
  ...
  return (
    <>
      ...
      >
        ...
        {eventLoader && creatorLoader && ratingLoader ? (
              ...
              >
                <div className="border-b-2 border-[#fff] border-opacity-10 flex flex-col justify-center lg:justify-start items-center w-full p-5">
                  ...
                  <p className="text-[#fff] w-full text-center lg:text-left mt-4">
                    Your feedback shapes the future! Let us know what you
                    thought of this event. Your insights will help us create
